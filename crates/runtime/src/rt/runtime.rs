/*
   ...
   Description: ... Summary ...
*/
use crate::{events::Events, procs::Processable};
...
impl Runtime {
    ...
    pub async fn spawn<T: Processable>(&mut self, proc: Arc<T>) -> AsyncResult<&Self>
    where
        T: 'static,
    {
        let mut i = 0;
...
mod tests {
    use super::*;
    use crate::procs::simple::SimpleProc;
    use std::sync::Arc;

    #[tokio::test]
    async fn test_default_rt() {
        let mut a = Runtime::default();
        let b = SimpleProc::default();
        assert!(a.spawn(Arc::new(b)).await.is_ok())
    }
}
