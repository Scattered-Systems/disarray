...
impl Context {
    ...
    fn process_writable(&mut self, peer_id: usize) -> std::io::Result<()> {
        ...
        match peer.writer.write() {
            ...
            Ok(WriteResult::ChanClosed) => {
                ...
                self.poll.registry().deregister(&mut peer.writer.queue)?;
            }
            Err(e) => {
                if e.kind() == std::io::ErrorKind::WouldBlock {
    ...
    fn listen(&mut self) -> std::io::Result<()> {
        ...
        self.poll.registry().register(
            &mut mio::net::TcpListener::bind(self.control_chan)?,
            CONTROL,
            mio::Interest::READABLE
        )?;
                            ...
                            match self.control_chan.try_recv() {
                                ...
                                Err(e) => match e {
                                    ...
                                    mpsc::TryRecvError::Disconnected => {
                                        ...
                                        self.poll.registry().deregister(&mut self.control_chan)?;
                                        break;
                                    }
                                },
                            }
